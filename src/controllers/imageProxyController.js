...
const pixivApiResponseValidator = (pixivApiResponse) => {
  ...
  } else if (pixivApiResponse.illust.image_urls.square_medium === "https://s.pximg.net/common/images/limit_unknown_360.png") {
    return {
      error_title: '404 Not Found',
      message_en: 'This work has been deleted or restricted.',
      message_zh: '這個作品可能已被刪除，或無法取得。',
    ...
    try {
        ...
        if (pixivApiResponseValidationResult !== null) {
          ...
          if (pixivApiResponse.illust.page_count === 1) {
            ...
            return;
          } else if (req.params.pageNumber > pixivApiResponse.illust.page_count) {
            res.status(404).render('error', {
              error_title: '404 Not Found',
          ...
          const imageURL = pixivApiResponse.illust.meta_pages[req.params.pageNumber - 1].image_urls.original;
          const imageResponse = await axios.get(imageURL, {
            headers: imageHeaders,
            responseType: 'stream',
          });
